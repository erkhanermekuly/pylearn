...
<style>
    ...
    /* Мобильді кесте үшін арнайы стиль */
...
<div class="lg:hidden flex justify-between items-center p-5 bg-white border-b sticky top-0 z-[60] shadow-sm">
    ...
    <div class="flex flex-col items-center text-center">
        <span class="font-black text-indigo-600 tracking-tighter text-sm uppercase">Ұстаз Панелі</span>
        <span id="mobileGroupName" class="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Журнал</span>
    </div>
    ...
    <div class="max-w-5xl mx-auto">
        ...
        @forelse ($lessons as $lesson)
            <div class="journal-card mb-8 overflow-hidden">
                ...
                <div class="p-5 sm:p-8 border-b border-slate-100 relative bg-white">
                    ...
                    <h3 class="text-lg sm:text-xl font-black text-slate-800 tracking-tight leading-tight">{{ $lesson->title }}</h3>
                    ...
                    <div class="flex flex-wrap gap-5 mt-6">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                <p class="text-[9px] uppercase font-black text-slate-400 leading-none">Студенттер</p>
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                <p class="text-[9px] uppercase font-black text-slate-400 leading-none">Жіберілді</p>
                ...
                <div class="overflow-x-auto">
                    <table class="w-full responsive-table">
                        <thead class="bg-slate-50 border-b border-slate-100">
                            <tr>
                                <th class="px-8 py-4 text-left text-slate-500 font-black text-[10px] uppercase tracking-widest">Студент</th>
                                <th class="px-8 py-4 text-center text-slate-500 font-black text-[10px] uppercase tracking-widest">Бағасы</th>
                                <th class="px-8 py-4 text-right text-slate-500 font-black text-[10px] uppercase tracking-widest">Статус</th>
                            </tr>
                        </thead>
                            ...
                            @foreach ($lesson->assignments as $assignment)
                                @forelse ($assignment->submissions as $submission)
                                    <tr class="hover:bg-slate-50/80 transition-colors bg-white">
                                        <td data-label="Студент" class="px-8 py-4 font-bold text-slate-700 text-sm">
                                            {{ $submission->user->name }}
                                        </td>
                                        <td data-label="Бағасы" class="px-8 py-4 text-center">
                                            ...
                                            @else
                                                <span class="text-slate-400 italic text-xs">Күтуде</span>
                                            @endif
                                        </td>
                                        <td data-label="Статус" class="px-8 py-4 text-right">
                                            @if ($submission->grade === null)
                                                <span class="badge bg-amber-100 text-amber-600">Тексерілуде</span>
                                            @else
                                                <span class="badge bg-emerald-100 text-emerald-600">Бағаланды</span>
                                            @endif
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="3" class="px-8 py-6 text-center text-slate-400 italic text-xs">Тапсырма әлі жіберілмеген</td>
                                    </tr>
                                @endforelse
                            @endforeach
                        </tbody>
                ...
                <div class="p-6 sm:p-8 bg-slate-50/50 border-t border-slate-100">
                    @if ($studentsWithNoSubmissions->isEmpty())
                        <div class="flex items-center gap-2 text-emerald-600 font-bold text-xs uppercase tracking-tighter">
                            ...
                            Барлық студент тапсырды
                        </div>
                    @else
                        <h4 class="text-[10px] font-black text-red-400 uppercase tracking-widest mb-3">Жібермегендер:</h4>
        ...
        @empty
            <div class="text-center py-16 journal-card bg-white">
                ...
                <p class="text-slate-400 font-black uppercase tracking-widest text-[10px]">Деректер табылған жоқ</p>
            </div>
        @endforelse
